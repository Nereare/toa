$(document).ready(function() {
  // Here be LAAAAAAAAAAAARGE arrays
  ...
  let sun = {
    "tropical": [ [360, 1080],  [360, 1080],  [360, 1080],  [360, 1080] ],
    "subtropical": [ [360, 1080],  [330, 1110],  [360, 1080],  [390, 1050] ],
    "temperate": [ [360, 1080],  [300, 1140],  [360, 1080],  [420, 1020] ],
  ...
  if ( localStorage.getItem("day") &&
    ...
    $("#setup-modal").addClass("is-active");
  }
  ...
  function updateTime() {
    ...
    let t = parseInt( $("#cal-time").val() );
    // The sun pattern is a string that is the index to the sunset/rise map
    let s = $("#cal-sun").val();
    ...
    let season = year[d][3];
    let ss = 0;
    if ( season == "Summer" ) { ss = 1; }
    else if ( season == "Autumn" ) { ss = 2; }
    else if ( season == "Winter" ) { ss = 3; }
    else { ss = 0; }
    // Get time of day
    let tod = "";
    if ( t >= sun[s][ss][1] ) { tod = "Night"; }
    else if ( t >= sun[s][ss][0] ) { tod = "Day"; }
    else { tod = "Night"; }
    ...
    $("#date-tod").html( tod );
    if ( year[d][4] != "" ) { $("#date-event").html( "&bull; " + year[d][4] ); }
    else { $("#date-event").html(""); }
    if ( year[d][0] == "0th" ) {
      $("#date-day").html("");
      $("#date-month").html( year[d][1] );
    } else {
      $("#date-day").html( year[d][0] );
  ...
  $(".button-remove").on("click", function() {
    let delta = parseInt( $(this).attr("data-delta") );
    let t = parseInt( $("#cal-time").val() );
    t -= delta;
    $("#cal-time").val(t).trigger("change");
  });
  // Build default add time button method
  ...
  // Build custom remove time button method
  $("#remove-custom").on("click", function() {
    if ( $("#remove-custom-delta").val() != "" && $("#remove-custom-unit").val() != "" ) {
      let delta = parseInt( $("#remove-custom-delta").val() ) * parseInt( $("#remove-custom-unit").val() );
      let t = parseInt( $("#cal-time").val() );
      t -= delta;
      $("#cal-time").val(t).trigger("change");
    }
  });
  // Build custom add time button method
  $("#add-custom").on("click", function() {
    if ( $("#add-custom-delta").val() != "" && $("#add-custom-unit").val() != "" ) {
      let delta = parseInt( $("#add-custom-delta").val() ) * parseInt( $("#add-custom-unit").val() );
      let t = parseInt( $("#cal-time").val() );
      t += delta;
      $("#cal-time").val(t).trigger("change");
    }
  ...
  // Set day when month is an event
  $("#setup-month").on("change", function() {
    let events = ["29", "120", "211", "272", "333"];
    if ( events.includes( $(this).val() ) ) {
      $("#setup-day")
        .val(1)
        .prop("disabled", true);
    } else {
      $("#setup-day").prop("disabled", false);
    }
  });
  // Show season for current selected day
  $("#setup-day, #setup-month").on("change input", function() {
    if ( $("#setup-day").val() != "" &&
         parseInt( $("#setup-day").val() ) > 0 &&
         parseInt( $("#setup-day").val() ) <= 30 &&
         $("#setup-month").val() != "" ) {
      let d = parseInt( $("#setup-day").val() ) + parseInt( $("#setup-month").val() );
      $("#setup-season").val( year[d][3] );
    }
  });
  // Set year name when one is given
  $("#setup-year").on("change input", function() {
    let y = $(this).val();
    if ( parseInt(y) >= -700 && parseInt(y) <= 1600 ) {
      $("#setup-year-name").val( year_name[y] );
    } else {
  ...
  $("#setup-do").on("click", function() {
    ...
    if ( $("#setup-day").val() != "" &&
      ...
      alert("Fill ALL fields, and make sure they have VALID values, please!");
    }
  });
